<?php
	...
	*                                                                          *
	* FILE HISTORY:                                                            *
	* Patch by Vlad Zaritsky 2014-11-28                                        *
	* -sha1 changed by md5 while temporary cache link generated                *
	*                                                                          *
	* Patch by Vlad Zaritsky 2014-12-02                                        *
	* -eliminate SQL error message on CIFS directory:                          *
	*  db-fail select custom_schema from simple_sys_tree where id='cifs:..'    *
...
function db_get_schema($schema_file, $tfolder="", $tview="", $cache=true, $popup=false) {
  static $data = array();
  if (!$cache) $data = array();

  //Patch by Vlad Zaritsky 2014-12-02 eliminate SQL error message like this:
...
function sys_cache_get_file($type, $id, $name, $create_dir=false) {
  $file = SIMPLE_CACHE."/".$type."/".sys_get_pathnum($id)."/".md5($id).urlencode($name);
  //sha1($id).urlencode($name);
